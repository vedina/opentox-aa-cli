...
public class aacli {
    ...
    enum policy_command {
	authorize, list, delete, create, archive
    }
	...
	if (ssotoken == null)
	    ssotoken = new OpenSSOToken(authService, suffix);
	...
	if (uri != null) {
		...
		try {
		    ...
		    if (code == 401)
			log(command, "Error: Only the policy creator can retrieve its content.");
		} catch (Exception x) {
	...
	case authorize: {
		...
		throw new MissingParameterException(String.format("%s>%s", command,
			"Missing URI. Have you specified an argument to the -r option?"));
	    log(command, String.format("URI: %s", uri));
	...
	case delete: {
		...
		else if (confirm(String.format("Do you really want to delete the policy %s ?", policyId))) {
		    deleteHandler.handlePolicy(policyId);
		} else
		...
		} else if (confirm(String.format("Do you really want to delete all policies for %s ?", uri))) {
		    OpenToxUser owner = new OpenToxUser();
		    policy.getURIOwner(ssotoken, uri, owner, deleteHandler);
		} else
		    throw new UserCancelledException();
		...
		} else if (confirm(String.format("Do you really want to delete all policies defined by %s?",
			user.getUsername()))) {
		    policy.listPolicies(ssotoken, deleteHandler);
		} else
		    throw new UserCancelledException();
	...
	case archive: {
	     ...
	     * {
	     * 
	     * @Override public boolean handlePolicy(String policyID) throws
	     * Exception { boolean ok = super.handlePolicy(policyID); long now =
	     * System.currentTimeMillis(); try { policy.listPolicy(ssotoken,
	     * policyID, this); now = System.currentTimeMillis() - now;
	     * log(command
	     * ,String.format("Policy '%s' retrieved and written in [%s ms]"
	     * ,policyID,now)); } catch (Exception x) {
	     * log(command,String.format
	     * ("ERROR retrieving policy %s",policyID)); } return ok; } };
	     */
	    listPolicies(policy, handler);
	    handler.close();
	...
	case create: {
		...
		throw new MissingParameterException(
			String.format("Directory %s does not exist!", dir.getAbsoluteFile()));
		...
		if (!confirm(String.format("ERROR reading %s! Is this a backup directory?",
			serversFile.getAbsoluteFile())))
		    throw new UserCancelledException();
		...
		try {
			...
			long now = System.currentTimeMillis();
			log(command,
				String.format("Preparing to send '%s' to the policy service at %s ...",
					file.getAbsoluteFile(), policy.getPolicyService()));
			if (confirmed
				|| confirm(String.format("Do you really want to create new policies at %s ?",
					ssotoken.getAuthService()))) {
			    confirmed = true;
	...
	default:
	    throw new MissingParameterException(String.format("%s not supported", command));
	}
	return 0;
    }
	...
	if (c != null) {
	    String confirm = c.readLine(String.format(
		    "Hope you know what are you doing.\n%s\nEnter Y, N or Q to quit:", message));
	    if ("Y".equals(confirm.trim().toUpperCase()))
		return true;
	    if ("Q".equals(confirm.trim().toUpperCase()))
		throw new UserCancelledException();
	}
	return true;
    }
	...
	case policyid: {
	    // if ((argument==null) || "".equals(argument.trim())) throw new
	    // IllegalArgumentException("Empty policy identifier!");
	...
	try {
	    ...
	    /*
	     * 
	     * File file = new File(input); if (file.exists()) { OpenToxAAcli
	     * cropper = new OpenToxAAcli(); if (file.isDirectory()) {
	     * cropper.crop(file.listFiles()); } else cropper.crop(file); } else
	...
	insecure {
		...
		Option option = OptionBuilder.withLongOpt(name()).withDescription(getDescription())
			.create(getShortName());
	...
	public Option createOption() {
	    ...
	    Option option = OptionBuilder
		    ...
		    .withDescription(
			    String.format("%s %s %s", getDescription(), defaultValue == null ? "" : "Default value: ",
				    defaultValue == null ? "" : defaultValue)).create(getShortName());
	...
	return "Retrieve all policies per URI:\n" + "\tjava -jar aacli\n"
	...
	return "Retriewe and store locally XML files for the user:\n" + "\tjava -jar aacli\n"
		+ "\t-n http://opensso.in-silico.ch/opensso/identity\n"
		+ "\t-z http://opensso.in-silico.ch/Pol/opensso-pol\n" + "\t-u guest\n" + "\t-p guest\n"
		+ "\t-b /tmp\n" + "\t-c archive";
	...
	return "Create new policies from a backup directory of XML files:\n" + "\tjava -jar aacli\n"
	...
	return "Retrieve policy content by policy id\n" + "\tjava -jar aacli\n"
		+ "\t-n http://opensso.in-silico.ch/opensso/identity\n"
		+ "\t-z http://opensso.in-silico.ch/Pol/opensso-pol\n" + "\t-u guest\n" + "\t-p guest\n"
	...
	return "Delete all policies per URI:\n" + "\tjava -jar aacli\n" + "\t-u guest\n" + "\t-p guest\n"
		+ "\t-r http://blabla.uni-plovdiv.bg:8080/ambit2/dataset/999\n" + "\t-c delete";

    }

    protected static String exampleDeletePolicyUser() {
	return "Delete all policies per user:\n" + "\tjava -jar aacli\n" + "\t-u guest\n" + "\t-p guest\n"
		+ "\t-c delete";

    }

    protected static String exampleDeletePolicy() {
	return "Delete policy by policy id:\n" + "\tjava -jar aacli\n" + "\t-u guest\n" + "\t-p guest\n"
